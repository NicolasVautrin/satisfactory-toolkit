...
export async function buildScenery() {
  ...
  // ── Batch-load resource node + fracking meshes ──────────
  ...
  const uniqueMeshNames = [...new Set([
    ...Object.values(RESOURCE_MESH),
    'SM_FrackingNode_Crack_01',
    'SM_FrackingNode_Mid_01',
  ])];

  // 1 batch for all resource/fracking meshes
  const meshCache = new Map(); // meshName → { geometry, material }
  const entries = await fetchBatchGlb(`scenery/${currentSceneryLod}`, uniqueMeshNames);
  for (const { name, glb } of entries) {
    const result = await parseSceneryGeometry(glb);
    if (result) meshCache.set(name, result);
  }
  ...
  for (let i = 0; i < meshNames.length; i += BATCH) {
    const batchNames = meshNames.slice(i, i + BATCH);
    const batchEntries = await fetchBatchGlb(`scenery/${currentSceneryLod}`, batchNames);

    // Parse all geometries in this batch
    const geomMap = new Map();
    for (const { name, glb } of batchEntries) {
      const result = await parseSceneryGeometry(glb);
      if (result) geomMap.set(name, result);
    }

    // Create InstancedMeshes
    for (const [meshName, result] of geomMap) {
      const instances = byMesh[meshName];
  ...
  const frackGeom = meshCache.get('SM_FrackingNode_Crack_01')?.geometry || fallbackGeom;
  const frackCoreGeom = meshCache.get('SM_FrackingNode_Mid_01')?.geometry || fallbackGeom;
...
async function parseSceneryGeometry(buffer) {
  try {
...
function createLandscapeProjectionMaterial() {
  ...
  return new THREE.ShaderMaterial({
    ...
    uniforms: {
      landscapeMap: { value: landscapeMap },
    ...
    fragmentShader: /* glsl */`
      ...
      void main() {
        ...
        uv = clamp(uv, 0.0, 1.0);
        vec3 color = pow(texture2D(landscapeMap, uv).rgb, vec3(1.0 / 2.2));

        vec3 lightDir = normalize(vec3(0.3, 0.2, 1.0));
        float diff = max(dot(normalize(vViewNormal), lightDir), 0.0);
        gl_FragColor = vec4(color * (0.4 + 0.6 * diff), 1.0);
      }
    `,
    lights: false,
  });
