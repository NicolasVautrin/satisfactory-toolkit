...
function reposition(platform, pos, rot) {
  ...
  if (platform.track) {
    ...
    const outward0 = { x: -trackDir.x, y: -trackDir.y, z: -(trackDir.z || 0) };
    platform.track.rebuildSpline(trackPos, outward0, trackEnd, trackDir);
  }
}
...
function dock(source, srcSide, target, tgtSide) {
  ...
  if (source.track && target.track) {
    ...
    source.track.connectPorts(srcPort, target.track, tgtPort);
  }
