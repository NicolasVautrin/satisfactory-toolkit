...
class BeltStation extends Builder {
  constructor(entity, components, track) {
    super(entity, components);
    this.track      = track;

    this._initPorts(components, PORTS);
    // Add non-port components (PlatformConnection, etc.) to _componentMap for dock()
    for (const c of components) {
      const short = c.instanceName.split('.').pop();
      if (!this._componentMap[short]) this._componentMap[short] = c;
    }
  }
  ...
  connectPorts(myPortName, other, otherPortName) {
    ...
    super.connectPorts(myPortName, other, otherPortName);
  }
