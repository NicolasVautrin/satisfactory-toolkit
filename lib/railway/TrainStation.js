...
class TrainStation extends Builder {
  constructor(entity, components, track, stationId) {
    ...
    // Expose track ports for the editor
    this._ports = {};
    if (this.track) {
      this._ports.TrackConnection0 = this.track._ports.TrackConnection0;
      this._ports.TrackConnection1 = this.track._ports.TrackConnection1;
    }
  }
  ...
  get label() {
    ...
    const prop = this.stationId.properties?.mStationName;
    return prop?.value?.value ?? prop?.value ?? null;
  }

  set label(name) {
    if (!this.stationId) throw new Error('No stationId on this TrainStation');
    if (!name) {
      delete this.stationId.properties.mStationName;
      return;
    }
    this.stationId.properties.mStationName = {
      type: 'TextProperty', ueType: 'TextProperty', name: 'mStationName',
      value: { flags: 18, historyType: 255, hasCultureInvariantString: true, value: name },
    };
  }
  ...
  connectPorts(myPortName, other, otherPortName) {
    if (myPortName.startsWith('TrackConnection') && other.track) {
      const RailwayHelper = require('./RailwayHelper');
      const mySide = myPortName === 'TrackConnection0' ? 0 : 1;
      const otherSide = otherPortName === 'TrackConnection0' ? 0 : 1;
      const myPlatComp = this._componentMap[`PlatformConnection${mySide}`];
      if (myPlatComp?.properties?.mConnectedTo) {
        throw new Error(`PlatformConnection${mySide} already connected`);
      }
      const otherPlatComp = other._componentMap?.[`PlatformConnection${otherSide}`];
      if (otherPlatComp?.properties?.mConnectedTo) {
        throw new Error(`PlatformConnection${otherSide} on target already connected`);
      }
      RailwayHelper.dock(other, otherSide, this, mySide);
      return;
    }
    // Track connection to a non-platform entity (e.g. standalone track)
    if (this.track) {
      this.track.connectPorts(myPortName, other, otherPortName);
      return;
    }
    throw new Error(`TrainStation ${this.inst}: cannot connect "${myPortName}"`);
  }
  ...
  static create(x, y, z, rotation = { x: 0, y: 0, z: 0, w: 1 }, opts = {}) {
    ...
    const trackTravel = new Vector3D(-1, 0, 0).rotate(rotation);
    const trackEnd    = trackPos.add(trackTravel.scale(TRACK_LENGTH));
    const outward0    = trackTravel.scale(-1);  // TC0 outward opposes travel
    const outward1    = trackTravel;             // TC1 outward = travel direction
    const track = RailroadTrack.create(
      { pos: trackPos, dir: outward0 },
      { pos: trackEnd, dir: outward1 },
      { integrated: true },
    );
