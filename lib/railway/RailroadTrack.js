...
class RailroadTrack extends SplineBuilder {
  ...
  snapPorts(srcPort, anchorPort) {
    ...
    this._trackSnap(srcPort, wPos, wDir);
  }

  /**
   * Override: snap to a free world position.
   * dir = outward direction of a virtual anchor (will be negated for opposition).
   */
  snapToPos(srcPort, pos, dir) {
    this._trackSnap(srcPort, pos, dir);
  }

  /**
   * Internal: track-specific snap logic. Negate dir, accumulate _snappedPorts,
   * rebuild spline + validate only when both endpoints have been snapped.
   */
  _trackSnap(snappedPort, wPos, wDir) {
    // Negate anchor dir for opposition convention
    const negDir = wDir ? { x: -wDir.x, y: -wDir.y, z: -(wDir.z||0) } : null;

    // Store world-space pos/dir on the snapped port
    snappedPort._wSnapPos = wPos;
    snappedPort._wSnapDir = negDir || snappedPort._dir;

    if (!this._snappedPorts) this._snappedPorts = new Set();
    this._snappedPorts.add(snappedPort._portName);

    // Rebuild + validate only when both ports have been snapped
    if (this._snappedPorts.size >= 2) {
      const p0 = this._ports[RailroadTrack.Ports.START];
      const p1 = this._ports[RailroadTrack.Ports.END];
      const wStartPos = p0._wSnapPos || p0.worldPos();
      const wStartDir = p0._wSnapDir || p0.worldDir();
      const wEndPos = p1._wSnapPos || p1.worldPos();
      const wEndDir = p1._wSnapDir || p1.worldDir();
      this.rebuildSpline(wStartPos, wStartDir, wEndPos, wEndDir);

      const { validateSplineShape } = require('../shared/validateSpline');
      validateSplineShape(this.entity, p0, p1);
    }
  }
  ...
  static create(from, to, opts = {}) {
    ...
    // Temporary spline for constructor to read port offsets
    entity.properties = {
      mSplineData:        wrapSplineData([
        splinePoint(new Vector3D(0, 0, 0), new Vector3D(1, 0, 0), new Vector3D(1, 0, 0)),
        splinePoint(new Vector3D(dx, dy, dz), new Vector3D(1, 0, 0), new Vector3D(1, 0, 0)),
      ]),
