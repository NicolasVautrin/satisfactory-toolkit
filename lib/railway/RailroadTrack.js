...
class RailroadTrack extends Builder {
  ...
  /**
   * Rebuild the rail spline.
   * When called with args (from snapTo): one port moved to wPos, other stays.
   * When called without args: _pos/_dir are world-space, set by caller.
   */
  onPortSnapped(snappedPort, wPos, wDir) {
    ...
    let wStartPos, wStartDir, wEndPos, wEndDir;
    if (!snappedPort) {
      wStartPos = p0._pos;
      wStartDir = p0._dir;
      wEndPos = p1._pos;
      wEndDir = p1._dir;
    } else if (snappedPort === p0) {
      wStartPos = wPos;
      wStartDir = p0.worldDir();
      wEndPos = p1.worldPos();
      wEndDir = p1.worldDir();
    } else {
      wEndPos = wPos;
      wEndDir = p1.worldDir();
      wStartPos = p0.worldPos();
      wStartDir = p0.worldDir();
    }
