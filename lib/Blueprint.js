  ...
  _findMatchingPorts(tolerance) {
    ...
    for (const [nodeId, machine] of Object.entries(this._nodes)) {
      ...
      for (const [portName, port] of Object.entries(machinePorts)) {
        if (!port.worldPos || port.isConnected) continue;
    ...
    for (let i = 0; i < allPorts.length; i++) {
      ...
      for (let j = i + 1; j < allPorts.length; j++) {
        ...
        const dist = new Vector3D(a.port.worldPos()).sub(new Vector3D(b.port.worldPos())).length;
        if (dist > tolerance) continue;

        if (a.port.worldDir && b.port.worldDir) {
          const wDirA = a.port.worldDir();
          const wDirB = b.port.worldDir();
          const dot = wDirA && wDirB ? new Vector3D(wDirA).dot(new Vector3D(wDirB)) : 0;
          if (dot >= 0) continue;
        }
