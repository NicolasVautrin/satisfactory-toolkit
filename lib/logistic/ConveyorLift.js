...
class ConveyorLift {
   ...
   * Called by FlowPort.snapTo() after setting port.pos/dir on the target port.
   * @param {FlowPort} snappedPort - the port whose pos/dir was just updated
   */
  recalcSpline(snappedPort) {
    ...
    // Rotate so port offset (+X local) faces opposite to the source port
    const rot = Quaternion.fromLocalToWorldZ(PORT_OFFSET_AXIS, opposedDir).toPlain();
    if (portName === 'bottom') {
  ...
  setPortDir(portName, dir) {
    const bottom = this._ports[ConveyorLift.Ports.BOTTOM];
    const top = this._ports[ConveyorLift.Ports.TOP];
    const rot = Quaternion.fromLocalToWorldZ(PORT_DIR_AXIS, dir).toPlain();
    if (portName === 'bottom') {
      this.entity.transform.rotation = rot;
