...
class ConveyorLift extends Builder {
  ...
  isSnappable(srcPort, anchorPort) {
    const portName = srcPort === this._ports[ConveyorLift.Ports.BOTTOM] ? 'bottom' : 'top';
    if (portName === 'bottom' && this._ports[ConveyorLift.Ports.TOP].isConnected) return false;
    return true;
  }
  ...
  snapPorts(srcPort, anchorPort) {
    ...
    const portName = srcPort === this._ports[ConveyorLift.Ports.BOTTOM] ? 'bottom' : 'top';
    const otherConnected = this._ports[portName === 'bottom' ? ConveyorLift.Ports.TOP : ConveyorLift.Ports.BOTTOM].isConnected;
