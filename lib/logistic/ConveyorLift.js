...
class ConveyorLift extends Builder {
  ...
  constructor(entity, comp0, comp1) {
    ...
    const bottom = new FlowPort(comp0, { x: ATTACH_DIST, y: 0, z: 0 }, { x: 0, y: -1, z: 0 });
    bottom.portType = PortType.BELT;
    bottom._owner = this;
    bottom._portName = 'bottom';
    bottom.flowType = null;

    const topPort = new FlowPort(comp1, { x: ATTACH_DIST, y: 0, z: 0 }, { x: 0, y: -1, z: 0 });
  ...
  setTopDir(lDir) {
    ...
    const topRot = Quaternion.fromLocalToWorldZ(PORT_OFFSET_AXIS, lDir).toPlain();
  ...
  onPortSnapped(snappedPort, wPos, wDir) {
    ...
    if (portName === 'top' && otherConnected) {
      ...
      this.setTopDir({ x: lOpposed.x, y: lOpposed.y });
      return;
    }
  ...
  attachLift(portName, liftConn) {
    ...
    if (fromPortName === 'bottom') {
      ...
      // Convert desired port dir to fromLift's entity-local space
      const fromRot = fromLift.entity.transform.rotation;
      const invFromRot = { x: 0, y: 0, z: -fromRot.z, w: fromRot.w };
      const lPortDir = new Vector3D(wOpposed.x, wOpposed.y, 0).rotateZ(invFromRot);

      // Convert port dir (local -Y) to arm dir (local +X): arm = {-portDir.y, portDir.x}
      const lArmDir = { x: -lPortDir.y, y: lPortDir.x };

      // Must be cardinal
      const isCardinal =
        (Math.abs(Math.abs(lArmDir.x) - 1) < 0.1 && Math.abs(lArmDir.y) < 0.1) ||
        (Math.abs(lArmDir.x) < 0.1 && Math.abs(Math.abs(lArmDir.y) - 1) < 0.1);
      if (!isCardinal) {
        throw new Error(
