...
class ConveyorLift extends Builder {
  ...
  onPortSnapped(snappedPort) {
    ...
    const otherName = snappedPort === this._ports[ConveyorLift.Ports.BOTTOM]
      ? ConveyorLift.Ports.TOP : ConveyorLift.Ports.BOTTOM;
    if (this._ports[otherName].isConnected) {
  ...
  attachBelt(portName, beltPort) {
    ...
    // The lift moves to align its port with the belt port (belt stays)
    const opposedDir = { x: -beltPort.dir.x, y: -beltPort.dir.y };
    const rot = Quaternion.fromLocalToWorldZ(PORT_DIR_AXIS, opposedDir).toPlain();
    if (portName === 'bottom') {
      this.entity.transform.rotation = rot;
    }
    port.dir = { x: opposedDir.x, y: opposedDir.y, z: 0 };
    this.setPosition(portName, beltPort.pos);
...
ConveyorLift.SNAP_BEHAVIOR = 'Se repositionne (port bottom) et tourne pour aligner son port sur le port cible. Snap cardinal uniquement (±X, ±Y).';
