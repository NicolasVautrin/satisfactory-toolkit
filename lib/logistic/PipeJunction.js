...
const PORT_OFFSETS = { '0': { x: 0, y: 0, z: 0 }, '1': { x: 0, y: 0, z: 0 }, '2': { x: 0, y: 0, z: 0 }, '3': { x: 0, y: 0, z: 0 } };
const PORT_DIRS = { '0': JUNCTION_DIRS[0], '1': JUNCTION_DIRS[1], '2': JUNCTION_DIRS[2], '3': JUNCTION_DIRS[3] };
...
class PipeJunction extends Builder {
  constructor(entity, connComps) {
    ...
    connComps.forEach((comp, i) => {
      const p = new FlowPort(comp, { x: 0, y: 0, z: 0 }, JUNCTION_DIRS[i]);
  ...
  attachPipe(pipe, position) {
    ...
    const jPort0 = this.port('0');
    const wJPort0Pos = jPort0.worldPos();
    const wJPort0Dir = jPort0.worldDir();
    conn1._pos = wJPort0Pos;
    conn1._dir = wJPort0Dir;
    conn1.wire(jPort0);
    pipe.onPortSnapped();

    // Create pipe2: junction port 1 (-X direction, backward) → original conn1 destination
    const jPort1 = this.port('1');
    const wJPort1Pos = jPort1.worldPos();
    const wJPort1Dir = jPort1.worldDir();
    ...
    pipe2Conn0._pos = wJPort1Pos;
    pipe2Conn0._dir = wJPort1Dir;
    pipe2Conn1._pos = wOrigConn1Pos;
    pipe2Conn1._dir = wOrigConn1Dir;
    pipe2Conn0.wire(jPort1);
