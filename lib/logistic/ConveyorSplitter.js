...
const SnapBuilder = require('../shared/SnapBuilder');
...
// Port layout declared before class so constructor can reference it
const PORT_LAYOUT = {
  Input1:  { offset: { x: -100, y: 0, z: 0 }, dir: { x: -1, y:  0, z: 0 }, flow: 'input',  type: PortType.BELT },
  Output1: { offset: { x:  100, y: 0, z: 0 }, dir: { x:  1, y:  0, z: 0 }, flow: 'output', type: PortType.BELT },
  Output2: { offset: { x: 0, y:  100, z: 0 }, dir: { x:  0, y:  1, z: 0 }, flow: 'output', type: PortType.BELT },
  Output3: { offset: { x: 0, y: -100, z: 0 }, dir: { x:  0, y: -1, z: 0 }, flow: 'output', type: PortType.BELT },
};

class ConveyorSplitter extends SnapBuilder {
  constructor(entity, components) {
    ...
    this._ports = {};
    for (const [name, def] of Object.entries(PORT_LAYOUT)) {
      const port = new FlowPort(this._componentMap[name], def.offset, def.dir);
...
ConveyorSplitter.PORT_LAYOUT = PORT_LAYOUT;
