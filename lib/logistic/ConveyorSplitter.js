...
class ConveyorSplitter extends Builder {
  ...
  onPortSnapped(snappedPort) {
    ...
    // Compute entity position from the snapped port's world position and local offset
    const portName = Object.entries(this._ports).find(([, p]) => p === snappedPort)?.[0];
    const localOffset = PORT_OFFSETS[portName];
    if (!localOffset) return;
    const rotation = this.entity.transform.rotation;
    const worldOffset = new Vector3D(localOffset).rotate(rotation);
    const entPos = new Vector3D(snappedPort.pos).sub(worldOffset);
  ...
  attachBelt(belt, position) {
    ...
    // Save original output info and detach
...
ConveyorSplitter.SNAP_BEHAVIOR = 'Position fixe. Lift se connecte verticalement sur ports alignes (meme XY).';
