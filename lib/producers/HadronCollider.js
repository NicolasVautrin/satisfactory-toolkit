...
const TYPE_PATH = '/Game/FactoryGame/Buildable/Factory/HadronCollider/Build_HadronCollider.Build_HadronCollider_C';
const RECIPE_BUILT = '/Game/FactoryGame/Recipes/Buildings/Recipe_HadronCollider.Recipe_HadronCollider_C';
...
const PORTS = {
  Input0:  { offset: { x: 1600, y: -1100, z: 100 }, dir: { x: 0, y: -1, z: 0 }, flow: 'input',  type: PortType.BELT },
  Input1:  { offset: { x: 1200, y: -1100, z: 100 }, dir: { x: 0, y: -1, z: 0 }, flow: 'input',  type: PortType.BELT },
  Output0: { offset: { x: 1400, y:  1110, z: 100 }, dir: { x: 0, y:  1, z: 0 }, flow: 'output', type: PortType.BELT },
  // Component name is 'FGPipeConnectionFactory' (not descriptive)
  FGPipeConnectionFactory: { offset: { x: 800, y: -1190, z: 175 }, dir: { x: 0, y: -1, z: 0 }, flow: 'input', type: PortType.PIPE },
};

class HadronCollider extends Builder {
  constructor(entity, components) {
    super(entity, components);
    this._initPorts(components, PORTS);
    this._powerConnTarget = new FlowPort(this._componentMap['PowerInput'], { x: 0, y: 0, z: 0 }, null);
    this._powerConnTarget.portType = PortType.POWER;
    this._powerConnTarget._owner = this;
  }
  ...
  static create(x, y, z, rotation = { x: 0, y: 0, z: 0, w: 1 }) {
    const id = nextId();
    const baseName = `Build_HadronCollider_C_${id}`;
    ...
    return new HadronCollider(entity, components);
  }
  ...
  static fromBlueprint(entity, blueprintTransform) {
    ...
    const machine = HadronCollider.create(worldTransform.translation.x, worldTransform.translation.y, worldTransform.translation.z, worldTransform.rotation);
...
HadronCollider.TYPE_PATH = TYPE_PATH;
HadronCollider.STACK_HEIGHT = STACK_HEIGHT;
HadronCollider.Ports = {
  INPUT0: 'Input0', INPUT1: 'Input1', OUTPUT0: 'Output0',
  PIPE_INPUT: 'FGPipeConnectionFactory',
};
HadronCollider.PORT_LAYOUT = PORTS;

module.exports = HadronCollider;