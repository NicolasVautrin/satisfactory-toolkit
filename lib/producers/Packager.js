...
const TYPE_PATH = '/Game/FactoryGame/Buildable/Factory/Packager/Build_Packager.Build_Packager_C';
const RECIPE_BUILT = '/Game/FactoryGame/Recipes/Buildings/Recipe_Packager.Recipe_Packager_C';
...
const PORTS = {
  ...
  Output1:           { offset: { x: 0, y:  300, z: 100 }, dir: { x: 0, y:  1, z: 0 }, flow: 'output', type: PortType.BELT },
  PipeInputFactory:  { offset: { x: 0, y: -380, z: 375 }, dir: { x: 0, y: -1, z: 0 }, flow: 'input',  type: PortType.PIPE },
  PipeOutputFactory: { offset: { x: 0, y:  350, z: 375 }, dir: { x: 0, y:  1, z: 0 }, flow: 'output', type: PortType.PIPE },
};

class Packager extends Builder {
  constructor(entity, components) {
    super(entity, components);
    this._initPorts(components, PORTS);
    this._powerConnTarget = new FlowPort(this._componentMap['PowerConnection'], { x: 0, y: 0, z: 0 }, null);
  ...
  static create(x, y, z, rotation = { x: 0, y: 0, z: 0, w: 1 }) {
    const id = nextId();
    const baseName = `Build_Packager_C_${id}`;
    ...
    const powerInfo = makePowerInfo(names.powerInfo, inst, 10);
    ...
    return new Packager(entity, components);
  }
  ...
  static fromBlueprint(entity, blueprintTransform) {
    ...
    const machine = Packager.create(worldTransform.translation.x, worldTransform.translation.y, worldTransform.translation.z, worldTransform.rotation);
...
Packager.TYPE_PATH = TYPE_PATH;
Packager.STACK_HEIGHT = STACK_HEIGHT;
Packager.Ports = {
  INPUT0: 'Input0', OUTPUT0: 'Output1',
  PIPE_INPUT: 'PipeInputFactory', PIPE_OUTPUT: 'PipeOutputFactory',
};
Packager.PORT_LAYOUT = PORTS;
