...
const TYPE_PATH = '/Game/FactoryGame/Buildable/Factory/ConstructorMk1/Build_ConstructorMk1.Build_ConstructorMk1_C';
const RECIPE_BUILT = '/Game/FactoryGame/Recipes/Buildings/Recipe_ConstructorMk1.Recipe_ConstructorMk1_C';
...
const PORTS = {
  ...
  Output0: { offset: { x: 0, y:  300, z: 100 }, dir: { x: 0, y:  1, z: 0 }, flow: 'output', type: PortType.BELT },
};
...
class Constructor extends Builder {
  ...
  static create(x, y, z, rotation = { x: 0, y: 0, z: 0, w: 1 }) {
    const id = nextId();
    const baseName = `Build_ConstructorMk1_C_${id}`;
    ...
    return new Constructor(entity, components);
  }

  static createStack(x, y, z, count, rotation = { x: 0, y: 0, z: 0, w: 1 }) {
    const stack = [];
    for (let i = 0; i < count; i++) {
      stack.push(Constructor.create(x, y, z + i * STACK_HEIGHT, rotation));
    }
    return stack;
  }
  ...
  static fromSave(entity, saveObjects) {
    ...
    const compNames = ['PowerInput', 'InventoryPotential', 'powerInfo',
      'InputInventory', 'OutputInventory', 'Input0', 'Output0', 'FGFactoryLegs'];
  ...
  static fromBlueprint(entity, blueprintTransform) {
    ...
    const machine = Constructor.create(worldTransform.translation.x, worldTransform.translation.y, worldTransform.translation.z, worldTransform.rotation);
...
Constructor.TYPE_PATH = TYPE_PATH;
Constructor.STACK_HEIGHT = STACK_HEIGHT;
Constructor.Ports = { INPUT0: 'Input0', OUTPUT0: 'Output0' };
Constructor.PORT_LAYOUT = PORTS;
