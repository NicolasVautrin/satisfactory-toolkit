...
function obbOverlap(a, b) {
  ...
  // Tolerance: boxes touching exactly (distance == ra+rb) are NOT overlapping.
  // Shrink each half-extent by SHRINK so that touching faces have a gap.
  const SHRINK = 1; // 1 UU (~1cm) tolerance

  // Shrunk half-extents for overlap tests
  const aHS = aHArr.map(h => Math.max(0, h - SHRINK));
  const bHS = bHArr.map(h => Math.max(0, h - SHRINK));

  // Test 3 axes of A
  for (let i = 0; i < 3; i++) {
    const ra = aHS[i];
    const rb = bHS[0] * absR[i][0] + bHS[1] * absR[i][1] + bHS[2] * absR[i][2];
    if (Math.abs(d.dot(aAxes[i])) > ra + rb) return false;
  }

  // Test 3 axes of B
  for (let j = 0; j < 3; j++) {
    const ra = aHS[0] * absR[0][j] + aHS[1] * absR[1][j] + aHS[2] * absR[2][j];
    const rb = bHS[j];
  ...
  // A0 × B0
  let ra = aHS[1] * absR[2][0] + aHS[2] * absR[1][0];
  let rb = bHS[1] * absR[0][2] + bHS[2] * absR[0][1];
  if (Math.abs(d.dot(aAxes[2]) * R[1][0] - d.dot(aAxes[1]) * R[2][0]) > ra + rb) return false;

  // A0 × B1
  ra = aHS[1] * absR[2][1] + aHS[2] * absR[1][1];
  rb = bHS[0] * absR[0][2] + bHS[2] * absR[0][0];
  if (Math.abs(d.dot(aAxes[2]) * R[1][1] - d.dot(aAxes[1]) * R[2][1]) > ra + rb) return false;

  // A0 × B2
  ra = aHS[1] * absR[2][2] + aHS[2] * absR[1][2];
  rb = bHS[0] * absR[0][1] + bHS[1] * absR[0][0];
  if (Math.abs(d.dot(aAxes[2]) * R[1][2] - d.dot(aAxes[1]) * R[2][2]) > ra + rb) return false;

  // A1 × B0
  ra = aHS[0] * absR[2][0] + aHS[2] * absR[0][0];
  rb = bHS[1] * absR[1][2] + bHS[2] * absR[1][1];
  if (Math.abs(d.dot(aAxes[0]) * R[2][0] - d.dot(aAxes[2]) * R[0][0]) > ra + rb) return false;

  // A1 × B1
  ra = aHS[0] * absR[2][1] + aHS[2] * absR[0][1];
  rb = bHS[0] * absR[1][2] + bHS[2] * absR[1][0];
  if (Math.abs(d.dot(aAxes[0]) * R[2][1] - d.dot(aAxes[2]) * R[0][1]) > ra + rb) return false;

  // A1 × B2
  ra = aHS[0] * absR[2][2] + aHS[2] * absR[0][2];
  rb = bHS[0] * absR[1][1] + bHS[1] * absR[1][0];
  if (Math.abs(d.dot(aAxes[0]) * R[2][2] - d.dot(aAxes[2]) * R[0][2]) > ra + rb) return false;

  // A2 × B0
  ra = aHS[0] * absR[1][0] + aHS[1] * absR[0][0];
  rb = bHS[1] * absR[2][2] + bHS[2] * absR[2][1];
  if (Math.abs(d.dot(aAxes[1]) * R[0][0] - d.dot(aAxes[0]) * R[1][0]) > ra + rb) return false;

  // A2 × B1
  ra = aHS[0] * absR[1][1] + aHS[1] * absR[0][1];
  rb = bHS[0] * absR[2][2] + bHS[2] * absR[2][0];
  if (Math.abs(d.dot(aAxes[1]) * R[0][1] - d.dot(aAxes[0]) * R[1][1]) > ra + rb) return false;

  // A2 × B2
  ra = aHS[0] * absR[1][2] + aHS[1] * absR[0][2];
  rb = bHS[0] * absR[2][1] + bHS[1] * absR[2][0];
