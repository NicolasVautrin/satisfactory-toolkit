...
/**
 * Check if a lightweight instance is alive (not demolished in-game).
 * The game clears usedRecipe+usedSwatchSlot but keeps the slot in the array.
 */
function isAlive(inst) {
  return !!(inst.usedRecipe && inst.usedRecipe.pathName);
}
...
function listStructuralTypes(lwSub) {
  ...
  return lwSub.specialProperties.buildables
    ...
    .map(b => ({
      ...
      count:    b.instances.filter(isAlive).length,
    }));
}
...
function getInstances(lwSub, typePath, sortByDistanceTo) {
  ...
  const instances = entry.instances.filter(isAlive).map(inst => ({
    instance: inst,
