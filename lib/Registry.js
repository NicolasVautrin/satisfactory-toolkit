...
class Registry {
  ...
  static default() {
    ...
    // Pipes — multi-tier
    const Pipe = require('./logistic/Pipe');
    for (const [, cfg] of Object.entries(Pipe.TIERS)) {
      const className = cfg.typePath.split('.').pop();
      _instance.registerAs(className, Pipe);
    }
    ...
    // Railway
    const RailroadTrack = require('./railway/RailroadTrack');
    _instance.registerAs('Build_RailroadTrack_C', RailroadTrack);
    _instance.registerAs('Build_RailroadTrackIntegrated_C', RailroadTrack);
    const TrainStation = require('./railway/TrainStation');
    const BeltStation = require('./railway/BeltStation');
    const PipeStation = require('./railway/PipeStation');
    _instance.registerAs('Build_TrainStation_C', TrainStation);
    _instance.registerAs('Build_TrainDockingStation_C', BeltStation);
    _instance.registerAs('Build_TrainDockingStationLiquid_C', PipeStation);
    const RailroadSignal = require('./railway/RailroadSignal');
    _instance.registerAs('Build_RailroadBlockSignal_C', RailroadSignal);
    _instance.registerAs('Build_RailroadPathSignal_C', RailroadSignal);
