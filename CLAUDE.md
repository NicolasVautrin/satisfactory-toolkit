...
- **Un restart du serveur perd tous les edits en mémoire** — la save est rechargée depuis le fichier original, les entités créées/modifiées/supprimées via `/api/game/edit` sont perdues. Si des edits étaient en cours, il faut les recréer après le restart
